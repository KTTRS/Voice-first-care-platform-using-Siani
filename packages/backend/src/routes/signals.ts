...
router.get('/high-priority', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    const signals = await prisma.signal.findMany({
      where: {
        severity: {
...
router.get('/analytics/:patientId', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    const { patientId } = req.params;
    ...
    const analytics = {
      ...
      averageScore: signals.length > 0 
        ...
        : 0,
      recentHighPriority: signals.filter(s => 
